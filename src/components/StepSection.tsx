...
/**
 * One row of the builder accordion. The design shows a "STEP X OF 4" label,
 * an icon, the step title, and either an open "N selected" count with an
 * up-chevron or a closed row with a down-chevron. Expanding a step reveals
 * its cards (or plan choices) and a "Next: …" button into whatever follows.
 */
export function StepSection({ step, index, nextStep }: Props) {
  ...
  /**
   * In the design, the "STEP X OF 4" eyebrow and the hairline above it sit
   * inside the same lavender surface as the header, flush with the top of
   * the review panel alongside it — so both live in this shared block.
   */
  const eyebrow = (
